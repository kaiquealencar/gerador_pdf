...
def gerar_holerite(nome, reg_sistema, reg_geral, cpf, conta):
    ...
    escrever(str(quinto_dia_util(now.year, now.month)), 418, 273)

    linha = 302
    for i in range(10):    
        escrever(periodo_mes(date.today().month), 388, linha)         
        linha += 12
